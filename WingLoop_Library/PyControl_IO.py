...
def read_aswing_file(filepath, data_dict, rename_map=None,
    ...
    with open(filepath, "r") as f:
        for iline,line in enumerate(f):
            if iline == 2:
                data_dict["ModelName"] = line.strip()
            ...
            # Extract variables
            __parse_line(data_dict, rename_map, pattern, line)
...
def __parse_line(data_dict, rename_map, pattern, line):
    matchcount =0
    for match in pattern.finditer(line):
        __extract_variable(data_dict, rename_map, match)
        matchcount +=1

def __extract_variable(data_dict, rename_map, match):
